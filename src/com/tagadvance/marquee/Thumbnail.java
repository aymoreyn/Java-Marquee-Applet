...
public class Thumbnail extends BufferedImage {
	...
	/**
	 * Scales <code>image</code> to <code>maxWidth</code>, <code>maxHeight</code>. The scaling used
	 * prevents landscape/portrait distortion, however, loss of image quality may occur due to
	 * shrinking.
	 */
	public Thumbnail(BufferedImage image, int maxWidth, int maxHeight, Color background,
			boolean center) {
	...
	/**
	 * Scales and crops <code>image</code>. <b>Warning:</b> actual image size may not match either
	...
	/**
	 * Forcibly resizes <code>image</code> to <code>size</code>. <b>Warning:</b> image may distort
	 * 
	 * @param image
	 * @param size
	 */
	...
	private static Dimension scale(BufferedImage image, double maxWidth, double maxHeight) {
